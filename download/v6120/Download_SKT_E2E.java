package osc_e2e.download.v6120;
...
*/
   ...
   driver.findElementByXPath("//android.widget.TextView[@text='알약M - 파일 청소 및 메신저 파일 정리, 국내 1위 무료 바이러스+스미싱 보안']").isDisplayed();   
   
   double endTimesearchEnter = System.nanoTime();
   System.out.println(endTimesearchEnter);
   double searchEnterResult = Double.parseDouble(String.format("%.2f",(((endTimesearchEnter - startTimesearchEnter)/1000000000) - totalThreadSleepTimesearch)));
   
   Thread.sleep(3000);
   driver.findElementByXPath("//android.widget.TextView[@text='알약M - 파일 청소 및 메신저 파일 정리, 국내 1위 무료 바이러스+스미싱 보안']").click(); // 상품명 선택
   ...
   driver.findElementByXPath("//android.widget.TextView[@text='알약M - 파일 청소 및 메신저 파일 정리, 국내 1위 무료 바이러스+스미싱 보안']").isDisplayed(); // 상품명 확인
   System.out.println("------goodsDetailenterComplete-------");
   
   double endTimeCompletein = System.nanoTime();
   System.out.println(endTimeCompletein);
   double CompleteEnterResult = Double.parseDouble(String.format("%.2f",(((endTimeCompletein - startTimeCompletein)/1000000000) - totalThreadSleepTime)));
   
   Thread.sleep(3000);
   driver.findElementById("com.skt.skaf.A000Z00040:id/text").click(); // 다운로드 선택
   
   //Thread.sleep(3000);
   //driver.findElementByXPath("//android.widget.TextView[@text='30MB가 넘는 상품을 데이터 네트워크로 다운로드하도록 요청하였습니다. 통화료가 발생할 수 있으니 주의하세요.']").isDisplayed();
   //driver.findElementById("com.skt.skaf.A000Z00040:id/buttonConfirm").click(); // 30MB 이상 다운로드 안내 팝업
   ...
   double CompletedownloadResult = Double.parseDouble(String.format("%.2f",(((endTimeCompletedownload - startTimeCompletedownload)/1000000000) - totalThreadSleepTimedownload)));
